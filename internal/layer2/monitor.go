...
func processLog(vLog types.Log, parsedABI abi.ABI) {
	...
	case parsedABI.Events["SubmitterRotationRequested"].ID.Hex():
		...
		if result.Error != nil {
			...
			submitterRotation.BlockNumber = vLog.BlockNumber
			submitterRotation.CurrentSubmitter = event.CurrentSubmitter.Hex()
			err = db.DB.Save(&submitterRotation).Error
		}
	...
	case parsedABI.Events["ParticipantAdded"].ID.Hex():
		...
		err = db.DB.FirstOrCreate(&participant, "address = ?", participant.Address).Error
		if err != nil {
	...
	case parsedABI.Events["ParticipantRemoved"].ID.Hex():
		...
		err = db.DB.Where("address = ?", event.Participant.Hex()).Delete(&db.Participant{}).Error
		if err != nil {
